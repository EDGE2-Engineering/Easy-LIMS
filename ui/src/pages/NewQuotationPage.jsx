    ...
    const loadFromSupabase = async (id) => {
      try {
        ...
        if (data && data.client_id) {
          ...
          if (clientData) data.clients = clientData;
        }
    ...
    if (jobId && !savedRecordId && clients.length > 0) {
      const loadJobDetails = async () => {
        try {
          ...
          const existingDocs = await apiClient.get('/api/documents', {
            params: { eq_job_id: jobId, eq_document_type: docType },
          });
  ...
  const handleSaveToDatabase = async () => {
    ...
    try {
      ...
      if (jobId && (!savedRecordId || isTypeChanged)) {
        ...
        if (targetStatus) {
          try {
            await apiClient.put(`/api/jobs/${jobId}`, { status: targetStatus });
          } catch (err) {
