    ...
    try {
      ...
      const [clientsData, jobsData] = await Promise.all([
        ...
        apiClient.get('/api/jobs', {
          params: {
            in_status: 'UNDER_TESTING,SENT_TO_TESTING_DEPARTMENT,TEST_COMPLETED',
            order_by: 'updated_at',
            order_dir: 'desc',
          },
        }),
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage and record test results for active lab jobs.
          </p>
        </div>
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <Badge
                    variant="outline"
                    className="font-mono text-xs text-primary bg-primary/5 border-primary/20"
                  >
                    {job.job_id}
                  </Badge>
                  ...
                  {Array.isArray(job.job_categories) &&
                    job.job_categories.map((cat) => (
                      <Badge
                        key={cat}
                        variant="secondary"
                        className="text-[9px] px-1.5 py-0 bg-gray-100 text-gray-600"
                      >
                        {cat}
                      </Badge>
                    ))}
