...
const TestingManager = ({ initialJobId, onClose, onSave }) => {
  ...
  const fetchData = async ({ silent = false } = {}) => {
    ...
    try {
      // Fetch Job
      const job = await apiClient.get(`/api/jobs/${initialJobId}`);
      if (job && job.client_id) {
        const clientData = await apiClient.get(`/api/clients/${job.client_id}`);
        if (clientData) job.clients = { client_name: clientData.client_name };
      }
      setJobDetails(job);

      // Fetch Samples
      const inwards = await apiClient.get('/api/material_inward_register', {
        params: { eq_job_id: initialJobId },
      });
      const inwardIds = (inwards || []).map((i) => i.id);
      let flatSamples = [];
      if (inwardIds.length > 0) {
        const allSamples = await apiClient.get('/api/material_samples', {
          params: { in_inward_id: inwardIds.join(',') },
        });
        flatSamples = allSamples || [];
      }
      ...
      if (!isNaN(userId)) {
        const caps = await apiClient.get('/api/technician_capabilities', {
          params: { eq_user_id: userId },
        });
        if (caps) setTechCapabilities(caps.map((c) => c.category));
      }
    } catch (error) {
      console.error(error);
  ...
  const handleSaveResults = async (category) => {
    ...
    try {
      ...
      const existingArr = await apiClient.get('/api/job_tests', {
        params: { eq_job_id: initialJobId, eq_category: category },
      });
      ...
      if (existing && existing.id) {
        await apiClient.put(`/api/job_tests/${existing.id}`, recordData);
        logAudit({
          userId,
          entityType: 'job_test',
        ...
        const data = await apiClient.post('/api/job_tests', recordData);
        if (data) {
          logAudit({
            userId,
            entityType: 'job_test',
