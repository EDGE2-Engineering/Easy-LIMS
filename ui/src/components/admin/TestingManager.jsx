...
const TestingManager = ({
  ...
  return (
    <div className="w-full animate-in fade-in duration-500 space-y-4">
          ...
          {visibleCategories.map((cat) => {
            ...
            return (
              <TabsContent key={cat} value={cat} className="space-y-6 outline-none mt-0">
                ...
                {isSolidHollowBlocks && (() => {
                  const solidHollowData = testResults[cat]?.SolidHollowBlocksData || testResults[cat]?.BlocksData;
                  return (
                    <div className="bg-white dark:bg-card p-6 rounded-none border border-gray-100 dark:border-border shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between h-full w-full col-span-full">
                      <div>
                        <div className="flex items-center justify-between mb-0">
                          <h4 className="text-sm font-bold text-gray-800 dark:text-foreground flex items-center gap-2">
                            {/* Solid / Hollow Blocks Test Data */}
                          </h4>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="outline" size="sm" onClick={() => setSolidHollowBlocksModalCategory(cat)} className="h-8 text-xs">
                                <Edit className="w-3 h-3 mr-1" />
                                {solidHollowData ? 'Edit Solid/Hollow Blocks Test Data' : 'Enter Solid/Hollow Blocks Test Data'}
                              </Button>
                            </TooltipTrigger>
                            <TooltipContent className="bg-gray-900 text-white border-gray-800">
                              <p className="text-xs">Open Solid/Hollow Blocks Test Input Modal per IS 2185 (Part 1)</p>
                            </TooltipContent>
                          </Tooltip>
                        </div>

                        {!solidHollowData ? (
                          <p className="text-xs text-gray-500 dark:text-muted-foreground mb-4">Pending solid/hollow blocks test input</p>
                        ) : (
                          <div className="space-y-4 mt-3">
                            {/* Summary row */}
                            <div className="flex flex-wrap gap-4 p-4 rounded-xl bg-stone-50/40 dark:bg-stone-900/20 border border-stone-200 dark:border-stone-700/40">
                              ...
                              {solidHollowData.compressiveStrength?.avgCompressiveStrength && (
                                <div>
                              ...
                              {solidHollowData.waterAbsorption?.avgWaterAbsorption && (
                                <div>
                                  <span className="text-[10px] uppercase font-bold text-blue-800 dark:text-blue-400">Avg Water Absorption</span>
                                  <div className="flex items-baseline gap-1">
                                    <span className="text-xl font-black text-blue-900 dark:text-blue-300 font-mono">
                              ...
                              {solidHollowData.blockDensity?.avgBlockDensity && (
                                <div>
      ...
      {/* Solid/Hollow Blocks Test Input Modal */}
      {solidHollowBlocksModalCategory && (
        <SolidHollowBlockTestModal
          isOpen={!!solidHollowBlocksModalCategory}
          onClose={() => setSolidHollowBlocksModalCategory(null)}
          jobCode={jobDetails?.job_code}
          sampleCode={
            samples.find(
              (s) =>
                String(s.material_type) === String(solidHollowBlocksModalCategory) ||
                materials.find((m) => String(m.id) === String(s.material_type))?.name ===
                  solidHollowBlocksModalCategory
            )?.sample_code || ''
          }
          initialData={
            testResults[solidHollowBlocksModalCategory]?.SolidHollowBlocksData ||
            testResults[solidHollowBlocksModalCategory]?.BlocksData ||
            {}
          }
          onApply={async (solidHollowBlocksData) => {
            const categoryToSave = solidHollowBlocksModalCategory;
            const updatedResults = {
              ...testResults,
              [categoryToSave]: {
                ...(testResults[categoryToSave] || {}),
                SolidHollowBlocksData: solidHollowBlocksData,
                BlocksData: solidHollowBlocksData,
              },
            };
            setTestResults(updatedResults);
