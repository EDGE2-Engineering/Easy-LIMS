...
import {
  ...
  SAMPLE_SOLID_HOLLOW_BLOCKS_TEST_DATA,
  calculateSolidHollowBlocksCompressiveStrength,
  calculateSolidHollowBlocksWaterAbsorption,
  calculateSolidHollowBlockDensity,
} from '@/utils/solidHollowBlocksTestCalculation';

function deepClone(o) { return JSON.parse(JSON.stringify(o)); }

// ─── Component ────────────────────────────────────────────────────────────────

export default function SolidHollowBlockTestModal({
  isOpen,
  onClose,
  sampleCode = '',
  jobCode = '',
  ...
  // ── Calculations ─────────────────────────────────────────────────────────
  const gradesList = SOLID_HOLLOW_BLOCK_GRADES || BLOCK_GRADES;
  const compResult = useMemo(() => calculateSolidHollowBlocksCompressiveStrength(compObs, grade), [compObs, grade]);
  const waResult   = useMemo(() => calculateSolidHollowBlocksWaterAbsorption(waObs),              [waObs]);
  const densResult = useMemo(() => calculateSolidHollowBlockDensity(densObs),                     [densObs]);
  ...
  const handleFillSample = () => {
    setGrade(SAMPLE_SOLID_HOLLOW_BLOCKS_TEST_DATA.grade);
    setSampleName(SAMPLE_SOLID_HOLLOW_BLOCKS_TEST_DATA.sampleName);
    setCompObs(deepClone(SAMPLE_SOLID_HOLLOW_BLOCKS_TEST_DATA.compressiveStrength.observations));
    setWaObs(deepClone(SAMPLE_SOLID_HOLLOW_BLOCKS_TEST_DATA.waterAbsorption.observations));
    setDensObs(deepClone(SAMPLE_SOLID_HOLLOW_BLOCKS_TEST_DATA.blockDensity.observations));
  };
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <DialogTitle className="text-base sm:lg font-bold text-gray-900 dark:text-foreground">
        ...
        <div className="shrink-0 p-4 border-t dark:border-border flex items-center justify-between gap-3 bg-gray-50/50 dark:bg-muted/20">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button type="button" size="sm" onClick={handleApply}
              ...
              <Check className="w-3.5 h-3.5" /> Save Solid/Hollow Blocks Test Data
            </Button>
          </div>
        </div>
...
export {
  SolidHollowBlockTestModal,
  SolidHollowBlockTestModal as BlocksTestModal,
  SolidHollowBlockTestModal as SolidHollowBlocksTestModal,
};
