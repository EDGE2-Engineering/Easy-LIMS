...
const InquiriesManager = () => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await apiClient.put(`/api/inquiries/${editingInquiry.id}`, {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/api/inquiries/${deleteConfirmation.inquiryId}`);
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setEditingInquiry((prev) => ({ ...prev, remarks: e.target.value }))
                }
  ...
  return (
    <div className="space-y-8 w-full pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1 uppercase text-[10px] tracking-widest ml-1">
            Track and respond to client inquiries and leads
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-2 px-4 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none border-l pl-3 ml-1">
              Showing {sortedInquiries.length === 0 ? 0 : startIndex + 1}-
              {Math.min(endIndex, sortedInquiries.length)} of {sortedInquiries.length}
            </span>
          </div>
      ...
      <AlertDialog
        ...
        onOpenChange={(isOpen) =>
          !isOpen && setDeleteConfirmation({ isOpen: false, inquiryId: null, clientName: '' })
        }
      >
        <AlertDialogContent className="rounded-3xl border-gray-100">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-500 mt-2">
              ...
              <span className="font-bold text-gray-900">{deleteConfirmation.clientName}</span>? This
          ...
          <AlertDialogFooter className="mt-6">
            ...
            <AlertDialogAction
              ...
              className="bg-red-600 hover:bg-red-700 text-white rounded-xl shadow-lg shadow-red-200 border-none"
            >
              Yes, Delete Inquiry
            </AlertDialogAction>
          </AlertDialogFooter>
