...
const MyLeaves = () => {
  ...
  const fetchUserLeaves = async () => {
    setLoading(true);
    try {
      const [usersData, requestData] = await Promise.all([
        apiClient.get('/api/users'),
        apiClient.get('/api/request_approvals', {
          params: {
            eq_requester_id: user.id,
      ...
      const enrichedLeaves = (requestData || []).map((r) => ({
        ...r,
  ...
  const handleCancelRequest = async () => {
    ...
    try {
      await apiClient.delete(`/api/request_approvals/${cancelTarget.id}`);
