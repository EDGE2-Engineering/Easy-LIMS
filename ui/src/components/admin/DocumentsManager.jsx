...
const DocumentsManager = () => {
  ...
  const fetchDocuments = async () => {
    setLoading(true);
    try {
      const [usersData, clientsData, jobsData, documentsData] = await Promise.all([
      ...
      const clientsMap = (clientsData || []).reduce((acc, c) => {
        acc[c.id] = { client_name: c.client_name, gstin: c.gstin };
        return acc;
      }, {});

      const jobsMap = (jobsData || []).reduce((acc, j) => {
        acc[j.id] = { project_name: j.project_name };
        return acc;
      }, {});

      let filteredDocs = (documentsData || []).map((d) => ({
        ...d,
        users: usersMap[d.created_by],
        clients: clientsMap[d.client_id],
        jobs: jobsMap[d.job_id],
      }));

      if (isStandard()) {
        filteredDocs = filteredDocs.filter((d) => d.created_by === user.id);
      }

      filteredDocs = filteredDocs.filter((d) => d.document_type !== 'Report');
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/api/documents/${deleteConfirmation.recordId}`);
