...
const LeavesManager = () => {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      const data = await apiClient.get('/api/users', {
        params: { neq_role: ROLES.SUPER_ADMIN.slug, order_by: 'full_name', order_dir: 'asc' },
      });
      setEmployees(data || []);
    } catch (error) {
  ...
  const fetchLeaves = useCallback(
    async (userId, year) => {
      ...
      try {
        const data = await apiClient.get('/api/request_approvals', {
          params: {
            eq_requester_id: userId,
            eq_request_type: 'LEAVE',
            eq_status: 'APPROVED',
          },
        });
  ...
  const handleSaveLeave = async () => {
    ...
    setIsSaving(true);
    try {
      await apiClient.post('/api/request_approvals', {
  ...
  const handleDeleteLeave = async () => {
    ...
    try {
      ...
      await apiClient.delete(`/api/request_approvals/${deleteTarget.request_id}`);
  ...
  const handleUpdateLeave = async () => {
    ...
    try {
      ...
      const currentReq = await apiClient.get(`/api/request_approvals/${editingLeave.request_id}`);
      ...
      await apiClient.put(`/api/request_approvals/${editingLeave.request_id}`, {
