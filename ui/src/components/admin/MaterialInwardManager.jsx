...
const MaterialInwardManager = ({ initialJobId, onClose, onSuccess }) => {
  ...
  const fetchCollectionCenters = async () => {
    try {
      const data = await apiClient.get('/api/collection_centers', {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const params = { order_by: 'created_at', order_dir: 'desc' };
      if (isStandard()) {
        params.eq_created_by = user.id;
      }

      const [data, clientsData, usersData, samplesData] = await Promise.all([
        apiClient.get('/api/material_inward_register', { params }),
        apiClient.get('/api/clients'),
        apiClient.get('/api/users'),
        apiClient.get('/api/material_samples'),
      ]);

      const clientsMap = clientsData.reduce((acc, c) => ({ ...acc, [c.id]: c.client_name }), {});
      const usersMap = usersData.reduce((acc, u) => ({ ...acc, [u.id]: u.full_name }), {});
      const samplesMap = samplesData.reduce((acc, s) => {
        if (!acc[s.inward_id]) acc[s.inward_id] = [];
        acc[s.inward_id].push({ received_date: s.received_date });
        return acc;
      }, {});

      const enrichedData = data.map((r) => ({
        ...r,
        clients: { client_name: clientsMap[r.client_id] },
        users: { full_name: usersMap[r.created_by] },
        material_samples: samplesMap[r.id] || [],
      }));
    ...
    if (initialJobId) {
      const checkExistingOrAddNew = async (jobId) => {
        ...
        try {
          ...
          const existingRecords = await apiClient.get('/api/material_inward_register', {
            params: { eq_job_id: jobId, order_by: 'created_at', order_dir: 'desc', limit: 1 },
          });
          ...
          const clientData = await apiClient.get(`/api/clients/${job.client_id}`);
          job.clients = clientData;
  ...
  const handleEdit = async (record) => {
    setLoading(true);
    try {
      const samples = await apiClient.get('/api/material_samples', {
        params: { eq_inward_id: record.id },
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isAddingNew) {
        ...
        const inwardData = await apiClient.post('/api/material_inward_register', {
        ...
        await apiClient.put(`/api/material_inward_register/${editingRecord.id}`, {
        ...
        const oldSamples = await apiClient.get('/api/material_samples', {
          params: { eq_inward_id: inwardId },
        });
        for (const os of oldSamples) {
          await apiClient.delete(`/api/material_samples/${os.id}`).catch(() => {});
        }
      }
      ...
      for (const st of samplesToInsert) {
        await apiClient.post('/api/material_samples', st);
      }
      ...
      if (editingRecord.job_id) {
        ...
        const jobData = await apiClient.get(`/api/jobs/${editingRecord.job_id}`);
        ...
        if (isAddingNew || (jobData && jobData.status === WORKFLOW_STATES.WORK_ORDER_RECEIVED)) {
          // Update job status
          await apiClient.put(`/api/jobs/${editingRecord.job_id}`, {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/api/material_inward_register/${deleteConfirmation.recordId}`);
