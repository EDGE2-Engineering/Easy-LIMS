...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const approvalsData = await apiClient.get('/api/request_approvals');
      ...
      const approvals = (approvalsData || []).map((r) => ({
        ...r,
      ...
      // 3. Fetch Total Staff
      const staffCount = (allUsers || []).filter((u) => u.role !== ROLES.SUPER_ADMIN.slug).length;

      // 4. Fetch Recent Activity (from workflow logs)
      const activityData = await apiClient.get('/api/job_workflow_logs', {
        params: { order_by: 'created_at', order_dir: 'desc', limit: 5 },
      });
      ...
      const activity = (activityData || []).map((a) => ({
        ...a,
        jobs: jobMap[a.job_id] || {},
        users: userMap[a.user_id] || {},
      }));

      // 5. Fetch Clients & Inquiries Stats
      const allClients = await apiClient.get('/api/clients');
      const clientsCount = (allClients || []).length;

      const allInquiries = await apiClient.get('/api/inquiries', {
        params: { eq_status: 'PENDING' },
      });
      ...
      const allDocsData = await apiClient.get('/api/documents');
      const allDocs = (allDocsData || []).filter((d) =>
        ['Quotation', 'Tax Invoice'].includes(d.document_type)
      );
      ...
      const clientMap = (allClients || []).reduce((acc, c) => {
        acc[c.id] = c;
        return acc;
      }, {});

      const todayDocs = (allDocsData || [])
        .filter((d) => d.created_at >= startOfTodayISO)
        .map((d) => ({
          ...d,
          clients: clientMap[d.client_id] || {},
          users: userMap[d.created_by] || {},
        }));
      const todayJobs = (jobs || [])
        .filter((d) => d.created_at >= startOfTodayISO)
        .map((j) => ({
          ...j,
          clients: clientMap[j.client_id] || {},
          users: userMap[j.created_by] || {},
        }));
      const todayExpenses = (expenses || [])
        .filter((d) => d.created_at >= startOfTodayISO)
        .map((e) => ({
          ...e,
          users: userMap[e.created_by] || {},
        }));
      const todayClients = (allClients || []).filter((c) => c.created_at >= startOfTodayISO);
