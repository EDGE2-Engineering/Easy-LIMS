...
const JobsManager = ({ id }) => {
  ...
  const fetchJobById = async (jobId) => {
    ...
    try {
      ...
      if (isNaN(userId)) {
        ...
        const u = (usersData || []).find((x) => x.username === user.username);
        if (u) userId = u.id;
      }
      ...
      if (data) {
        if (data.client_id) {
          const clientData = await apiClient.get(`/api/clients/${data.client_id}`);
          if (clientData) {
            data.clients = {
        ...
        if (
          ...
          const assignments = await apiClient.get('/api/job_to_technicians', {
            params: { eq_job_id: actualJobId, eq_technician_id: userId },
          });
  ...
  const handleReceiveWorkOrder = async () => {
    ...
    try {
      ...
      await apiClient.put(`/api/jobs/${editingRecord.id}`, {
        work_order_id: woId,
  ...
  const fetchJobDocs = async (jobId) => {
    try {
      const data = await apiClient.get('/api/documents', { params: { eq_job_id: jobId } });
      setLinkedDocs(data || []);
    } catch (error) {
  ...
  const fetchClients = async () => {
    try {
      const data = await apiClient.get('/api/clients', {
        params: { order_by: 'client_name', order_dir: 'asc' },
      });
      setClients(data || []);
    } catch (err) {
  ...
  const fetchJobAssignments = async (jobId) => {
    try {
      const [assignments, users] = await Promise.all([
        apiClient.get('/api/job_to_technicians', { params: { eq_job_id: jobId } }),
      ...
      const techs = (assignments || []).map((r) => usersMap[r.technician_id]).filter(Boolean);
  ...
  const fetchJobSamples = async (jobId) => {
    try {
      const inwardRecords = await apiClient.get('/api/material_inward_register', {
        params: { eq_job_id: jobId, order_by: 'created_at', order_dir: 'desc' },
      });

      if (inwardRecords && inwardRecords.length > 0) {
        const inwardIds = inwardRecords.map((r) => r.id);
        const [samplesData, usersData, ccData] = await Promise.all([
          apiClient.get('/api/material_samples', { params: { in_inward_id: inwardIds.join(',') } }),
          apiClient.get('/api/users'),
          apiClient.get('/api/collection_centers'),
        ]);
        ...
        const ccMap = (ccData || []).reduce((acc, c) => {
          acc[c.id] = c;
          return acc;
        }, {});

        const enrichedSamples = (samplesData || []).map((s) => ({
          ...s,
          users: usersMap[s.received_by] ? { full_name: usersMap[s.received_by].full_name } : null,
          collection_centers: ccMap[s.collection_center_id]
            ? { name: ccMap[s.collection_center_id].name }
            : null,
        }));

        setJobSamples(enrichedSamples);
      } else {
        setJobSamples([]);
      }
    } catch (err) {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const [jobsData, clientsData, usersData] = await Promise.all([
        apiClient.get('/api/jobs', {
          params: { order_by: 'created_at', order_dir: 'desc', limit: 10000 },
        }),
        apiClient.get('/api/clients', { params: { limit: 10000 } }),
        apiClient.get('/api/users'),
      ]);
      ...
      filteredJobs = filteredJobs.map((j) => ({
        ...j,
        clients: clientsMap[j.client_id]
          ? {
              client_name: clientsMap[j.client_id].client_name,
              client_address: clientsMap[j.client_id].client_address,
              gstin: clientsMap[j.client_id].gstin,
            }
          : null,
        users: usersMap[j.created_by] ? { full_name: usersMap[j.created_by].full_name } : null,
      }));
      ...
      if (!isAdmin() && user?.role !== ROLES.MRO.slug) {
        ...
        if (user?.role === ROLES.ANALYST.slug || user?.role === ROLES.TECHNICIAN.slug) {
          const assignments = await apiClient.get('/api/job_to_technicians', {
            params: { eq_technician_id: userId },
          });
          const assignedJobIds = (assignments || []).map((a) => a.job_id);
          filteredJobs = filteredJobs.filter((j) => assignedJobIds.includes(j.id));
        } else {
          filteredJobs = filteredJobs.filter((j) => j.created_by === userId);
        }
      }
      ...
      let quotationMap = {};
      if (jobIds.length > 0) {
        // Handle large number of jobIds by splitting if necessary, or just fetch all quotations
        const docs = await apiClient.get('/api/documents', {
          params: { eq_document_type: 'Quotation', limit: 10000 },
        });
        (docs || []).forEach((doc) => {
          if (jobIds.includes(doc.job_id)) {
  ...
  const handleGenerateReport = async () => {
    try {
      const testData = await apiClient.get('/api/job_tests', {
        params: { eq_job_id: editingRecord.id },
      });
  ...
  const handleSaveReport = async () => {
    ...
    try {
      ...
      if (existingReport) {
        await apiClient.put(`/api/documents/${existingReport.id}`, docPayload);
        logAudit({
          userId,
          entityType: 'document',
        ...
        const data = await apiClient.post('/api/documents', docPayload);
        logAudit({
          userId,
          entityType: 'document',
          entityId: data?.id,
  ...
  const fetchAuditLogs = async (jobId) => {
    ...
    try {
      const [logsData, usersData] = await Promise.all([
        apiClient.get('/api/job_workflow_logs', {
          params: { eq_job_id: jobId, order_by: 'created_at', order_dir: 'desc' },
        }),
        apiClient.get('/api/users'),
      ]);
      ...
      const enrichedLogs = (logsData || []).map((l) => ({
        ...l,
        users: usersMap[l.performed_by]
          ? {
              full_name: usersMap[l.performed_by].full_name,
              username: usersMap[l.performed_by].username,
            }
          : null,
      }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!clientId || isNaN(clientId)) throw new Error('Please select a valid client.');
      ...
      if (isAddingNew) {
        ...
        const insertData = { ...payload, created_by: userId, updated_by: userId };
        const data = await apiClient.post('/api/jobs', insertData);
        logAudit({
          userId,
          entityType: 'job',
          entityId: data?.id,
          entityName: data?.job_code,
          action: 'CREATE',
        });
      } else {
        let userId = typeof user.id === 'string' ? parseInt(user.id) : user.id;
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (inwardRecords && inwardRecords.length > 0) {
        const inwardIds = inwardRecords.map((r) => r.id);
        const materialSamples = await apiClient.get('/api/material_samples', {
          params: { in_inward_id: inwardIds.join(',') },
        });
        for (const sample of materialSamples || []) {
          await apiClient.delete(`/api/material_samples/${sample.id}`);
        }
        for (const i_id of inwardIds) {
          await apiClient.delete(`/api/material_inward_register/${i_id}`);
        }
      }

      const jobTests = await apiClient.get('/api/job_tests', { params: { eq_job_id: jobId } });
      for (const t of jobTests || []) {
        await apiClient.delete(`/api/job_tests/${t.id}`);
      }

      const workflowLogs = await apiClient.get('/api/job_workflow_logs', {
        params: { eq_job_id: jobId },
      });
      for (const l of workflowLogs || []) {
        await apiClient.delete(`/api/job_workflow_logs/${l.id}`);
      }

      const docs = await apiClient.get('/api/documents', { params: { eq_job_id: jobId } });
      for (const d of docs || []) {
