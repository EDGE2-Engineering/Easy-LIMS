...
const ApprovalsManager = () => {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      // We need to fetch related users and clients for manual joins
      ...
      const usersMap = (usersData || []).reduce((acc, u) => {
        acc[u.id] = { id: u.id, full_name: u.full_name, username: u.username, role: u.role };
        return acc;
      }, {});

      const clientsMap = (clientsData || []).reduce((acc, c) => {
        acc[c.id] = { id: c.id, client_name: c.client_name };
        return acc;
      }, {});

      // 1. Fetch Request Approvals (Leaves, etc)
      const requestDataRaw = await apiClient.get('/api/request_approvals', {
        params: { eq_status: filter, order_by: 'created_at', order_dir: 'desc' },
      });

      const requestData = (requestDataRaw || []).map((r) => ({
        ...r,
        requester: usersMap[r.requester_id],
        reviewer: usersMap[r.reviewed_by],
      }));

      // 2. Fetch Jobs Pending Review (if filter is PENDING)
      let jobRequests = [];
      if (filter === 'PENDING') {
        const jobsRaw = await apiClient.get('/api/jobs', {
          params: { eq_status: WORKFLOW_STATES.TEST_DATA_UNDER_REVIEW },
        });

        jobRequests = (jobsRaw || []).map((job) => ({
          id: `job-${job.id}`,
          real_id: job.id,
          request_type: 'JOB_REVIEW',
          status: 'PENDING',
          created_at: job.updated_at || job.created_at,
          requester: usersMap[job.created_by],
          request_data: {
            job_code: job.job_code,
            project_name: job.project_name,
            client_name: clientsMap[job.client_id]?.client_name,
            id: job.id,
          },
        }));
      }
  ...
  const handleAction = async (request, action, remarks) => {
    try {
      if (request.request_type === 'JOB_REVIEW') {
        ...
        // Update Job Status
        await apiClient.put(`/api/jobs/${request.real_id}`, {
          status: targetState,
        ...
        await apiClient.put(`/api/request_approvals/${request.id}`, {
          status: status,
          reviewed_by: user.id,
