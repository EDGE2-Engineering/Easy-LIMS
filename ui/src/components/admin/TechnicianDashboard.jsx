...
const TechnicianDashboard = () => {
      ...
      if (assignments && assignments.length > 0) {
        ...
        const [jobsData, clientsData] = await Promise.all([
          apiClient.get('/api/jobs', {
            params: { in_id: jobIds.join(','), order_by: 'created_at', order_dir: 'desc' },
          }),
        ...
        jobs = jobsData.map((j) => ({
          ...j,
          clients: { client_name: clientsMap[j.client_id] },
        }));
      }
  ...
  const testingJobsCount = assignedJobs.filter(
    (j) => j.status === WORKFLOW_STATES.UNDER_TESTING
  ).length;
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="space-y-8 pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            {
              label: 'Currently Under Testing',
              value: testingJobsCount,
              icon: Beaker,
              path: '#/settings/jobs?status=UNDER_TESTING',
              tooltip: 'View jobs currently under testing',
            },
          ].map((stat, idx) => (
