...
const AdminCompanyCalendar = () => {
  ...
  const fetchEvents = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiClient.get('/api/company_calendar', {
        params: { order_by: 'event_date', order_dir: 'asc' },
      });
  ...
  const fetchLeaves = useCallback(async () => {
    try {
      const [usersData, allRequests] = await Promise.all([
        ...
        apiClient.get('/api/request_approvals'),
      ]);

      const usersMap = (usersData || []).reduce((acc, u) => {
        acc[u.id] = { full_name: u.full_name, username: u.username };
        return acc;
      }, {});
      ...
      const approvedLeaves = enrichedRequests.filter(
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      ...
      if (editingEvent) {
        await apiClient.put(`/api/company_calendar/${editingEvent.id}`, eventData);
        logAudit({
        ...
        const response = await apiClient.post('/api/company_calendar', eventData);
        const data = [response];
        logAudit({
          userId: currentUser?.id,
          entityType: 'calendar_event',
  ...
  const handleDeleteEvent = async () => {
    ...
    setIsSaving(true);
    try {
      await apiClient.delete(`/api/company_calendar/${editingEvent.id}`);
