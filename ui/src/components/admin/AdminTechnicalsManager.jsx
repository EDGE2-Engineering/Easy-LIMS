...
const AdminTechnicalsManager = () => {
  ...
  const filteredTechnicals = technicals.filter(
    (t) =>
  ...
  const handleSave = async () => {
    if (!editingTech.text.trim()) {
      toast({
        ...
        description: 'Technical text cannot be empty.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      if (isAddingNew) {
        ...
        toast({
          title: 'Technical Added',
          description: 'New technical entry has been successfully added.',
        });
      } else {
      ...
      toast({
        title: 'Error',
        description: 'Failed to save technical entry.',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
  ...
  const confirmDelete = async () => {
    if (deleteConfirmation.id) {
      try {
        ...
        toast({
          title: 'Technical Deleted',
          description: 'The technical entry has been removed.',
          variant: 'destructive',
        });
      } catch (error) {
        ...
        toast({
          title: 'Error',
          description: 'Failed to delete technical entry.',
          variant: 'destructive',
        });
      }
    }
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm animate-in slide-in-from-right-4 duration-300">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">
            {isAddingNew ? 'Add New Technical' : 'Edit Technical'}
          </h2>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1 uppercase text-[10px] tracking-widest ml-1">
            Manage technical procedures and standards
          </p>
        </div>
      </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this technical entry? This action cannot be undone.
