...
const AdminCollectionCentersManager = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isAddingNew) {
        const response = await apiClient.post('/api/collection_centers', {
        ...
        await apiClient.put(`/api/collection_centers/${editingCenter.id}`, {
          name: editingCenter.name,
          address: editingCenter.address,
  ...
  const confirmDelete = async () => {
    if (deleteConfirmation.id) {
      try {
        await apiClient.delete(`/api/collection_centers/${deleteConfirmation.id}`);
        logAudit({
          userId: currentUser?.id,
          entityType: 'collection_center',
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm animate-in slide-in-from-right-4 duration-300">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">
            {isAddingNew ? 'Add New Collection Center' : 'Edit Collection Center'}
          </h2>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1 uppercase text-[10px] tracking-widest ml-1">
            Manage sample collection points and locations
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Tooltip>
          ...
          <TooltipContent className="bg-gray-900 text-white border-gray-800">
            <p className="text-xs">Register a new sample collection location</p>
          </TooltipContent>
        </Tooltip>
      </div>
