...
const MroDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    setLoading(true);
    try {
      const [clientsData, allInwards, allSamples, allJobs] = await Promise.all([
      ...
      const clientsMap = (clientsData || []).reduce((acc, c) => {
        acc[c.id] = { client_name: c.client_name };
        return acc;
      }, {});

      // 1 & 2. Counts
      const inwardCount = (allInwards || []).length;
      const samplesCount = (allSamples || []).length;

      // 3. Fetch recent inwards (for the list)
      const inwards = (allInwards || [])
        .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
        .slice(0, 10)
        .map((i) => ({ ...i, clients: clientsMap[i.client_id] }));
      setRecentInwards(inwards);

      // 4. Fetch jobs awaiting material (WORK_ORDER_RECEIVED status)
      const readyJobs = (allJobs || [])
        .filter((j) => j.status === WORKFLOW_STATES.WORK_ORDER_RECEIVED)
        .sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at))
        .map((j) => ({ ...j, clients: clientsMap[j.client_id] }));
