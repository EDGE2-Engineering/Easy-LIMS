...
const AccountsDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const docsData = await apiClient.get('/api/documents', {
        params: { eq_created_by: userId, order_by: 'created_at', order_dir: 'desc' },
      });
      const clientsData = await apiClient.get('/api/clients');
      ...
      const readyJobsData = await apiClient.get('/api/jobs', {
        params: {
          eq_status: WORKFLOW_STATES.REPORT_SIGNED,
          order_by: 'updated_at',
          order_dir: 'desc',
        },
      });
      const readyJobs = (readyJobsData || []).map((j) => ({
        ...j,
      ...
      const allDocsData = await apiClient.get('/api/documents', {
        params: { eq_document_type: 'Tax Invoice' },
      });
      const existingInvoices = allDocsData || [];
      ...
      const todayClients = (clientsData || [])
        .filter((c) => c.created_at >= todayStart.toISOString())
      ...
      const allExpenses = await apiClient.get('/api/expenses');
      const expThisMonth = (allExpenses || []).filter(
        (e) => e.date >= firstOfMonth && e.date <= lastOfMonth
      );
