...
const NewReportForm = ({ editReport, onCancel, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchJobOrders = async () => {
      try {
        if (!user) return;
        const [inwardData, clientsData] = await Promise.all([
          apiClient.get('/api/material_inward_register'),
          apiClient.get('/api/clients'),
        ]);
        const clientsMap = (clientsData || []).reduce((acc, c) => {
          acc[c.id] = c;
          return acc;
        }, {});
        const data = (inwardData || []).map((r) => ({
          job_order_no: r.job_order_no,
          client_id: r.client_id,
          clients: clientsMap[r.client_id]
            ? {
                client_name: clientsMap[r.client_id].client_name,
                client_address: clientsMap[r.client_id].client_address,
              }
            : null,
        }));
  ...
  const getInitialFormData = () => {
    ...
    return {
      ...
      isCodes: [
        ...
        {
          key: 'Qualitative Classification System of Rock Mass',
          value: 'IS:13365 - (Part 1) - 1998',
        },
      ],
      surveyReport: [
  ...
  const generateRandomSampleData = () => {
    const randomProjectTypes = [
      'Commercial Building (G+4)',
      'Residential Apartment (Stilt+10)',
      'Industrial Shed',
      'Bridge Abutment',
      'Overhead Water Tank',
    ];
    ...
    const data = {
      ...
      recommendations:
        'Based on the investigation, isolated foundation is recommended. Protective drainage should be provided. Fill material should be well compacted.',
    ...
    data.subSoilProfile = data.boreholeLogs.map((bh) =>
      bh.slice(0, 2).map((log, i) => ({
        depth:
          i === 0
            ? `0.0 to ${log.toDepth || log.depth}`
            : `${bh[i - 1].toDepth || bh[i - 1].depth} to ${log.toDepth || log.depth}`,
  ...
  const fillWithRandomData = () => {
    if (
      window.confirm(
        'This will fill the form with random sample data and overwrite any current changes. Proceed?'
      )
    ) {
  ...
  const validate = () => {
    ...
    // Basic Info Tab
    if (!formData.projectType || formData.projectType === '__other__')
      newErrors.projectType = { message: 'Project Type is required', tab: 'basic' };
    if (!formData.reportId) newErrors.reportId = { message: 'Report ID is required', tab: 'basic' };
    if (!formData.projectDetails)
      newErrors.projectDetails = { message: 'Project Details are required', tab: 'basic' };
    if (!formData.jobOrderNumber)
      newErrors.jobOrderNumber = { message: 'Job Order Number is required', tab: 'basic' };
    ...
    if (!formData.surveyDate)
      newErrors.surveyDate = { message: 'Survey Date is required', tab: 'basic' };

    // Survey Tab
    if (!formData.depthOfFoundation)
      newErrors.depthOfFoundation = { message: 'Depth of Foundation is required', tab: 'survey' };
    if (!formData.recommendationTypes.rock && !formData.recommendationTypes.soil) {
      newErrors.recommendationTypes = {
  ...
  const handleSoilSearch = (e, levelIndex, logIndex) => {
    ...
    const filtered = soilTypes.filter((type) => type.toLowerCase().includes(value.toLowerCase()));
  ...
  const removeChemicalAnalysisKey = (index, keyIndex) => {
    ...
    if (newAnalysis[index].additionalKeys.length > 1) {
      newAnalysis[index].additionalKeys = newAnalysis[index].additionalKeys.filter(
        (_, i) => i !== keyIndex
      );
      setFormData((prev) => ({
        ...prev,
  ...
  const removeDirectShearReading = (levelIndex, testIndex, readingIndex) => {
    if (formData.directShearResults[levelIndex][testIndex].stressReadings.length > 1) {
      const newResults = [...formData.directShearResults];
      newResults[levelIndex][testIndex].stressReadings = newResults[levelIndex][
        testIndex
      ].stressReadings.filter((_, i) => i !== readingIndex);
      setFormData((prev) => ({
        ...prev,
        directShearResults: newResults,
      }));
  ...
  const removePointLoadReading = (levelIndex, testIndex, readingIndex) => {
    if (formData.pointLoadStrength[levelIndex][testIndex].readings.length > 1) {
      const newResults = [...formData.pointLoadStrength];
      newResults[levelIndex][testIndex].readings = newResults[levelIndex][
        testIndex
  ...
  const handleSaveOnly = async () => {
    ...
    try {
      ...
      const existingArr = await apiClient.get('/api/reports', {
        params: { eq_report_number: formData.reportId },
      });
  ...
  const executeSave = async (isGenerating, existingRecord = null) => {
    try {
      ...
      if (isNaN(userId) && user.username) {
        ...
        const userData = (usersData || []).find((x) => x.username === user.username);
      ...
      if (existingRecord || formData.id) {
        ...
        await apiClient.put(`/api/reports/${updateId}`, payload);
      } else {
      ...
      toast({
        ...
        description: isGenerating
          ? 'Report saved and PDF generated successfully.'
          : 'The report data has been successfully saved to the database.',
        className: 'bg-green-50 border-green-200 text-green-900',
      });
      ...
      toast({
        title: 'Error',
        description: 'Failed to process report: ' + error.message,
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-xl border border-gray-100 shadow-sm gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button type="submit" form="report-form" disabled={isSaving} className="hidden">
            Generate Report
          </Button>
        </div>
      </div>
            ...
            <form id="report-form" onSubmit={handleSubmit} className="space-y-8">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="bg-white p-1 border border-gray-200 rounded-xl shadow-sm h-auto inline-flex mb-8 w-full justify-center">
                  <TabsTrigger
                    value="basic"
                    className="px-3 py-2 rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white transition-all flex items-center gap-2 relative"
                  >
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <Label
                          htmlFor="projectType"
                          className={errors.projectType ? 'text-red-500' : ''}
                        >
                          Project Type
                        </Label>
                        <div className="space-y-2">
                          <Select
                            value={
                              ['tower'].includes(formData.projectType)
                            ...
                            onValueChange={(value) => {
                              if (value === 'others') {
                                if (
                                  ...
                                  handleChange({
                                    target: { name: 'projectType', value: '__other__' },
                                  });
                                }
                              } else {
                          ...
                          {!['tower'].includes(formData.projectType) && formData.projectType && (
                            <Input
                              value={
                                formData.projectType === '__other__' ? '' : formData.projectType
                              }
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="reportId" className={errors.reportId ? 'text-red-500' : ''}>
                          Report ID
                        </Label>
                        <Input
                          id="reportId"
                        ...
                        />
                        {errors.reportId && (
                          <p className="text-xs text-red-500 mt-1">{errors.reportId.message}</p>
                        )}
                      </div>

                      <div className="space-y-2 relative">
                        <Label
                          htmlFor="jobOrderNumber"
                          className={errors.jobOrderNumber ? 'text-red-500' : ''}
                        >
                          Job Order Number
                        </Label>
                      ...
                      <div className="col-span-full space-y-2">
                        <Label
                          ...
                          className={errors.projectDetails ? 'text-red-500' : ''}
                        >
                          Project Details
                        </Label>
                        <Input
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="siteId" className={errors.siteId ? 'text-red-500' : ''}>
                          Site ID
                        </Label>
                        <Input
                          id="siteId"
                        ...
                        />
                        {errors.siteId && (
                          <p className="text-xs text-red-500 mt-1">{errors.siteId.message}</p>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="siteName" className={errors.siteName ? 'text-red-500' : ''}>
                          Site Name
                        </Label>
                        <Input
                          id="siteName"
                      ...
                      <div className="space-y-2">
                        <Label
                          htmlFor="surveyDate"
                          className={errors.surveyDate ? 'text-red-500' : ''}
                        >
                          Survey Date
                        </Label>
                        <AppDatePicker
                        ...
                        />
                        {errors.surveyDate && (
                          <p className="text-xs text-red-500 mt-1">{errors.surveyDate.message}</p>
                        )}
                      </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      ...
                      {formData.isCodes.map((code, index) => (
                        <div
                          key={index}
                          className="grid grid-cols-12 gap-4 items-start bg-gray-50/50 p-0 rounded-md"
                        >
                ...
                <TabsContent value="survey" className="mt-0 space-y-8">
                      ...
                      <div className="flex items-center space-x-2 mb-4">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setFormData((prev) => ({ ...prev, includeSurveyReportNote: checked }))
                          }
                        />
                        <label
                            ...
                            >
                              <div className="col-span-5">
                                <Input
                                  ...
                                  value={item.key}
                                  onChange={(e) =>
                                    handleSurveyReportChange(index, 'key', e.target.value)
                                  }
                                  className="bg-white"
                                />
                              </div>
                              <div className="col-span-6">
                                <Textarea
                                  ...
                                  value={item.value}
                                  onChange={(e) =>
                                    handleSurveyReportChange(index, 'value', e.target.value)
                                  }
                                  className="min-h-[40px] bg-white resize-y"
                                  rows={1}
                                />
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {formData.conclusions.map((item, index) => (
                        <div
                          key={index}
                          className="flex gap-4 items-start bg-gray-50/50 p-0 rounded-md group"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      {errors.depthOfFoundation && (
                        <p className="text-xs text-red-500 mt-1">
                          {errors.depthOfFoundation.message}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div id="recommendationTypes">
                    <h3
                      className={`text-lg font-semibold mb-4 pb-2 border-b border-gray-100 ${errors.recommendationTypes ? 'text-red-500 border-red-500' : 'text-gray-900'}`}
                    >
                      {' '}
                      Recommendation Type/s For SBC
                    </h3>
                    <div className="flex items-center space-x-8 pt-2">
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor="rec-rock" className="cursor-pointer">
                          Rock
                        </Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor="rec-soil" className="cursor-pointer">
                          Soil
                        </Label>
                      </div>
                    </div>
                    {errors.recommendationTypes && (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              Borehole Log - Level {levelIndex + 1}
                            </h4>
                                  ...
                                  >
                                        ...
                                        onFocus={() => {
                                          ...
                                          setFilteredSoilTypes(
                                            soilTypes.filter((type) =>
                                              type
                                                .toLowerCase()
                                                .includes((log.soilType || '').toLowerCase())
                                            )
                                          );
                                        }}
                                        onBlur={() =>
                                          setTimeout(() => setShowSoilSuggestions(false), 200)
                                        }
                                        className="h-8"
                                          ...
                                          <div className="absolute z-[100] w-64 bg-white border rounded-md shadow-lg max-h-40 overflow-auto mt-1 left-0">
                                            {filteredSoilTypes.map((type, idx) => (
                                              <div
                                                ...
                                                onClick={() =>
                                                  selectSoilType(type, levelIndex, logIndex)
                                                }
                                              >
                                                {type}
                                              </div>
                                    ...
                                    <td className="px-2 py-2">
                                      ...
                                      <Input
                                        ...
                                        className="h-8"
                                        placeholder="SPT at 45cm"
                                      />
                                    </td>
                                    <td className="px-2 py-2">
                                      <Input
                                        value={log.shearParameters?.cValue}
                                        onChange={(e) =>
                                          handleBoreholeLogChange(
                                            levelIndex,
                                            logIndex,
                                            'shearParameters.cValue',
                                            e.target.value
                                          )
                                        }
                                        className="h-8 mb-1"
                                      ...
                                      />
                                      <Input
                                        value={log.shearParameters?.phiValue}
                                        onChange={(e) =>
                                          handleBoreholeLogChange(
                                            levelIndex,
                                            logIndex,
                                            'shearParameters.phiValue',
                                            e.target.value
                                          )
                                        }
                                        className="h-8"
                    ...
                    <div className="space-y-8">
                      {formData.labTestResults.map((levelLogs, levelIndex) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              Lab Test Result - Level {levelIndex + 1}
                            </h4>
                              ...
                              <tbody>
                                {levelLogs.map((result, logIndex) => (
                                  ...
                                  >
                                    ...
                                    <td className="px-2 py-2">
                                      <Input
                                        value={result.grainSizeDistribution.gravel}
                                        onChange={(e) =>
                                          handleLabTestResultChange(
                                            levelIndex,
                                            logIndex,
                                      ...
                                      />
                                      <Input
                                        value={result.grainSizeDistribution.sand}
                                        onChange={(e) =>
                                          handleLabTestResultChange(
                                            levelIndex,
                                            logIndex,
                                      ...
                                      />
                                      <Input
                                        value={result.grainSizeDistribution.siltAndClay}
                                        onChange={(e) =>
                                          handleLabTestResultChange(
                                            levelIndex,
                                            logIndex,
                                        ...
                                        className="h-8"
                                        placeholder="Silt and Clay (%)"
                                      />
                                    </td>
                                    <td className="px-2 py-2">
                                      <Input
                                        value={result.atterbergLimits.liquidLimit}
                                        onChange={(e) =>
                                          handleLabTestResultChange(
                                            levelIndex,
                                            logIndex,
                                      ...
                                      />
                                      <Input
                                        value={result.atterbergLimits.plasticLimit}
                                        onChange={(e) =>
                                          handleLabTestResultChange(
                                            levelIndex,
                                            logIndex,
                                      ...
                                      />
                                      <Input
                                        value={result.atterbergLimits.plasticityIndex}
                                        onChange={(e) =>
                                          handleLabTestResultChange(
                                            levelIndex,
                                            logIndex,
                    ...
                    <div className="space-y-8">
                      {formData.chemicalAnalysis.map((item, index) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              Chemical Analysis - Level {index + 1}
                            </h4>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                              <div className="space-y-2">
                                ...
                                <Input
                                  value={item.phValue}
                                  onChange={(e) =>
                                    handleChemicalAnalysisChange(index, 'phValue', e.target.value)
                                  }
                              ...
                              <div className="space-y-2">
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    handleChemicalAnalysisChange(index, 'sulphates', e.target.value)
                                  }
                              ...
                              <div className="space-y-2">
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    handleChemicalAnalysisChange(index, 'chlorides', e.target.value)
                                  }
                                ...
                                >
                                  <div className="col-span-5">
                                    <Input
                                      ...
                                      value={keyItem.key}
                                      onChange={(e) =>
                                        handleChemicalAnalysisKeyChange(
                                          index,
                                          keyIndex,
                                          'key',
                                          e.target.value
                    ...
                    <div className="space-y-8">
                      {formData.grainSizeAnalysis.map((levelRows, levelIndex) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              Grain Size Analysis - Level {levelIndex + 1}
                            </h4>
                                    ...
                                    <td className="px-2 py-2 text-center">
                                      {levelRows.length > 1 && (
                                        <Button
                                          ...
                                          size="icon"
                                          onClick={() =>
                                            removeGrainSizeAnalysisRow(levelIndex, rowIndex)
                                          }
                                          className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-50"
                                        >
                                          <Trash2 className="w-4 h-4" />
                    ...
                    <div className="space-y-8">
                      {formData.sbcDetails.map((levelRows, levelIndex) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              SBC Details - Level {levelIndex + 1}
                            </h4>
                            ...
                            <table className="w-full text-sm text-left border-collapse min-w-[600px] rounded-lg">
                              <thead className="text-xs text-gray-700 uppercase bg-gray-100 border-b">
                                <tr>
                                  ...
                                  <th className="px-3 py-3 w-[150px] text-center">
                                    Use for report?
                                  </th>
                ...
                <TabsContent value="borehole" className="mt-0 space-y-8">
                  ...
                  <div className="bg-gray-50/50 p-6 rounded-lg border border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b border-gray-100">
                      Sub Soil Profile and Classifications
                    </h3>
                    <div className="space-y-8">
                      {formData.subSoilProfile.map((levelRows, levelIndex) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              Sub Soil Profile - Level {levelIndex + 1}
                            </h4>
                                    ...
                                    <td className="px-2 py-2 text-center">
                                      {levelRows.length > 1 && (
                                        <Button
                                          ...
                                          size="icon"
                                          onClick={() =>
                                            removeSubSoilProfileRow(levelIndex, rowIndex)
                                          }
                                          className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-50"
                                        >
                                          <Trash2 className="w-4 h-4" />
                    ...
                    <div className="space-y-8">
                      {formData.directShearResults.map((levelTests, levelIndex) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              Direct Shear - Level {levelIndex + 1}
                            </h4>
                ...
                <TabsContent value="pointload" className="mt-0 space-y-8">
                                    ...
                                    <tbody>
                                      {test.readings.map((item, readingIndex) => (
                                        ...
                                        >
                                          <td className="px-2 py-2">
                                            <Input
                                              value={item.loadAtFailure}
                                              onChange={(e) =>
                                                handlePointLoadReadingChange(
                                                  levelIndex,
                                                  testIndex,
                                                  readingIndex,
                                                  'loadAtFailure',
                  ...
                  <div className="bg-gray-50/50 p-6 rounded-lg border border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b border-gray-100">
                      Point Load Strength Index of Lump
                    </h3>
                    <div className="space-y-8">
                      {formData.pointLoadStrengthLump.map((levelTests, levelIndex) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              Point Load (Lump) - Level {levelIndex + 1}
                            </h4>
                                        ...
                                        >
                                          <td className="px-2 py-2">
                                            <Input
                                              value={item.loadAtFailure}
                                              onChange={(e) =>
                                                handlePointLoadLumpReadingChange(
                                                  levelIndex,
                                                  testIndex,
                                                  readingIndex,
                                                  'loadAtFailure',
                    ...
                    <div className="space-y-8">
                      {formData.foundationRockFormations.map((levelData, levelIndex) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-md font-semibold text-gray-700">
                              Foundation (Rock) - Level {levelIndex + 1}
                            </h4>
                              ...
                              <tbody>
                                {levelData.rows.map((item, rowIndex) => (
                                  ...
                                  >
                                    ...
                                    <td className="px-2 py-2">
                                      <Input
                                        value={item.spacingDiscontinuity}
                                        onChange={(e) =>
                                          handleFoundationRockRowChange(
                                            levelIndex,
                                            rowIndex,
                                    ...
                                    <td className="px-2 py-2">
                                      <Input
                                        value={item.conditionOfDiscontinuity}
                                        onChange={(e) =>
                                          handleFoundationRockRowChange(
                                            levelIndex,
                                            rowIndex,
                                    ...
                                    <td className="px-2 py-2">
                                      <Input
                                        value={item.discontinuityOrientation}
                                        onChange={(e) =>
                                          handleFoundationRockRowChange(
                                            levelIndex,
                                            rowIndex,
                                    ...
                                    <td className="px-2 py-2 text-center">
                                      {levelData.rows.length > 1 && (
                                        <Button
                                          ...
                                          size="icon"
                                          onClick={() =>
                                            removeFoundationRockRow(levelIndex, rowIndex)
                                          }
                                          className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-50"
                                        >
                                          <Trash2 className="w-4 h-4" />
      ...
      {isPreviewOpen && (
        <ReportPreview formData={formData} onClose={() => setIsPreviewOpen(false)} />
      )}
      {/* Overwrite Confirmation Dialog */}
      <AlertDialog
        open={saveConfirmation.isOpen}
        onOpenChange={(isOpen) =>
          !isOpen && setSaveConfirmation({ isOpen: false, isGenerating: false })
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-semibold text-gray-900">{formData.reportId}</span> already
              exists. Saving will overwrite the existing data. Are you sure you want to continue?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
