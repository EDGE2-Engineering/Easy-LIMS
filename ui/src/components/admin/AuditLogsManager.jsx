...
const AuditLogsManager = () => {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const allAudit = await apiClient.get('/api/audit_logs');
      const allWorkflow = await apiClient.get('/api/job_workflow_logs');
      const total = (allAudit?.length || 0) + (allWorkflow?.length || 0);

      const todayStart = new Date();
      todayStart.setHours(0, 0, 0, 0);

      const todayAuditCount = (allAudit || []).filter(
        (a) => new Date(a.created_at) >= todayStart
      ).length;
      const todayWorkflowCount = (allWorkflow || []).filter(
        (a) => new Date(a.created_at) >= todayStart
      ).length;
      const todayCount = todayAuditCount + todayWorkflowCount;

      const allPerformers = [
        ...(allAudit || []).map((r) => r.performed_by),
        ...(allWorkflow || []).map((r) => r.performed_by),
      ].filter(Boolean);
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      const usersMap = (usersData || []).reduce((acc, u) => {
        acc[u.id] = { id: u.id, full_name: u.full_name, username: u.username };
        return acc;
      }, {});
      ...
      if (!filterActivityType || filterActivityType !== 'job_workflow') {
        let params = { order_by: 'created_at', order_dir: 'desc', limit: 500 };
        if (filterUser) params.eq_performed_by = filterUser;
        if (filterActivityType) params.eq_entity_type = filterActivityType;

        const data = await apiClient.get('/api/audit_logs', { params });
        let filteredData = data || [];

        if (filterDateFrom) {
          const start = new Date(filterDateFrom).getTime();
          filteredData = filteredData.filter((d) => new Date(d.created_at).getTime() >= start);
        }
        if (filterDateTo) {
          const end = new Date(filterDateTo);
          end.setHours(23, 59, 59, 999);
          filteredData = filteredData.filter(
            (d) => new Date(d.created_at).getTime() <= end.getTime()
          );
        }

        // manual join
        auditRows = filteredData.map((d) => ({ ...d, users: usersMap[d.performed_by] }));
      }
      ...
      if (!filterActivityType || filterActivityType === 'job_workflow') {
        ...
        const data = await apiClient.get('/api/job_workflow_logs', { params });
        let filteredData = data || [];
        ...
        if (filterState) {
          filteredData = filteredData.filter(
            (d) => d.from_state === filterState || d.to_state === filterState
          );
        }

        const jobsData = await apiClient.get('/api/jobs');
        const jobsMap = (jobsData || []).reduce((acc, j) => {
          acc[j.id] = { job_code: j.job_code, project_name: j.project_name };
          return acc;
        }, {});

        workflowRows = filteredData.map((d) => ({
          ...d,
          users: usersMap[d.performed_by],
          jobs: jobsMap[d.job_id],
        }));
      }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      let auditRows = [];
      if (!filterActivityType || filterActivityType !== 'job_workflow') {
        let params = { order_by: 'created_at', order_dir: 'desc' };
        if (filterUser) params.eq_performed_by = filterUser;
        if (filterActivityType) params.eq_entity_type = filterActivityType;
