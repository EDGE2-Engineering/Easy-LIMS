    ...
    const updateSampling = useCallback(async (updatedItem) => {
        ...
        try {
            ...
            if (updatedItem.tcList?.length > 0) {
                const { data: terms } = await supabase.from('terms_and_conditions').select('id').in('type', updatedItem.tcList);
                if (terms?.length > 0) {
                    await supabase.from('sampling_to_terms_conditions').insert(terms.map(t => ({ sampling_id: id, tc_id: t.id })));
                }
            }
            ...
            if (updatedItem.techList?.length > 0) {
                const { data: techs } = await supabase.from('technicals').select('id').in('type', updatedItem.techList);
                if (techs?.length > 0) {
                    await supabase.from('sampling_to_technicals').insert(techs.map(t => ({ sampling_id: id, technical_id: t.id })));
                }
            }
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                if (newItem.tcList?.length > 0) {
                    const { data: terms } = await supabase.from('terms_and_conditions').select('id').in('type', newItem.tcList);
                    if (terms?.length > 0) {
                        await supabase.from('sampling_to_terms_conditions').insert(terms.map(t => ({ sampling_id: id, tc_id: t.id })));
                    }
                }
                ...
                if (newItem.techList?.length > 0) {
                    const { data: techs } = await supabase.from('technicals').select('id').in('type', newItem.techList);
                    if (techs?.length > 0) {
                        await supabase.from('sampling_to_technicals').insert(techs.map(t => ({ sampling_id: id, technical_id: t.id })));
                    }
                }
