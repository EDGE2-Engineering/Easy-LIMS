...
import { 
    Calculator, RotateCcw, TrendingUp, Info, Save, Loader2, IndianRupee,
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-1">
                ...
                <p className="text-gray-500 text-sm font-medium">Helpful tools and calculators for daily operations</p>
            </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                ...
                <TabsContent value="wage_calculator" className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <Select value={formData.year} onValueChange={(val) => setFormData(p => ({ ...p, year: val, totalWorkingDays: '', daysWorked: '' }))}>
                                        <SelectTrigger className="h-11 rounded-xl bg-gray-50/50 border-gray-100 font-bold"><SelectValue /></SelectTrigger>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select value={formData.month} onValueChange={(val) => setFormData(p => ({ ...p, month: val, totalWorkingDays: '', daysWorked: '' }))}>
                                        <SelectTrigger className="h-11 rounded-xl bg-gray-50/50 border-gray-100 font-bold"><SelectValue /></SelectTrigger>
                                        <SelectContent className="rounded-xl">{months.map((m, i) => <SelectItem key={i} value={i.toString()}>{m}</SelectItem>)}</SelectContent>
                                    </Select>
                                </div>
                            </div>

                            <div className="space-y-2">
                                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Monthly Salary (Base)</Label>
                                <Input 
                                    type="number" 
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Working Days</Label>
                                    <Input type="number" value={formData.totalWorkingDays} onChange={e => setFormData(p => ({ ...p, totalWorkingDays: e.target.value }))} className="h-11 rounded-xl bg-gray-50/50 border-gray-100 font-bold" />
                                    <p className="text-[10px] text-gray-400 font-medium italic">Usually excluding Sundays.</p>
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Days Worked</Label>
                                    <Input type="number" value={formData.daysWorked} onChange={e => setFormData(p => ({ ...p, daysWorked: e.target.value }))} className="h-11 rounded-xl bg-gray-50/50 border-gray-100 font-bold" />
                                    <p className="text-[10px] text-gray-400 font-medium italic">Employee's actual attendance.</p>
                                </div>
                            </div>

                            <Button onClick={handleCalculate} className="w-full bg-primary hover:bg-primary-dark text-white rounded-2xl h-14 font-black text-sm shadow-xl shadow-primary/20 mt-4">
                            ...
                            {showCalculation ? (
                                <div className="bg-gradient-to-br from-primary to-green-500 p-10 rounded-[2.5rem] shadow-2xl shadow-primary/20 text-white h-full flex flex-col justify-center relative overflow-hidden animate-in zoom-in-95 duration-500">
                                    ...
                                    <div className="relative z-10 space-y-2 text-center mb-10">
                                        <h2 className="text-sm font-black opacity-80 uppercase tracking-[0.2em]">Final Wage Payable</h2>
                                        <div className="text-7xl font-black tracking-tighter drop-shadow-md">₹{calculatedWage !== null ? calculatedWage.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00'}</div>
                                        <p className="text-xs font-bold opacity-60 mt-2 uppercase tracking-widest">For {months[parseInt(formData.month)]} {formData.year}</p>
                                    </div>
                                    ...
                                    <div className="relative z-10 space-y-4 pt-8 border-t border-white/20">
                                        <div className="flex justify-between items-center bg-white/10 p-3 rounded-2xl backdrop-blur-sm">
                                            ...
                                            <span className="font-black">{formData.daysWorked} / {formData.totalWorkingDays} Days</span>
                                        </div>
                                        <div className="flex justify-between items-center bg-white/10 p-3 rounded-2xl backdrop-blur-sm">
                                            <span className="text-xs font-bold opacity-80 uppercase">Daily Rate</span>
                                            <span className="font-black">₹{(parseFloat(formData.salary) / parseFloat(formData.totalWorkingDays)).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span>
                                        </div>
                                    </div>
                                    
                                    <div className="mt-10 flex items-center gap-4 relative z-10">
                                        <button onClick={handleResetCalculator} className="flex-grow rounded-2xl bg-white text-primary font-black px-6 py-4 shadow-xl hover:shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all">
                                            Calculate New
                                        </button>
                                        <button onClick={handleResetCalculator} title="Reset All" className="w-14 h-14 flex items-center justify-center rounded-2xl bg-white/20 hover:bg-white/30 text-white transition-all backdrop-blur-md">
                                ...
                                <div className="h-full min-h-[400px] flex flex-col items-center justify-center p-10 bg-gray-50 border-2 border-dashed border-gray-200 rounded-[2.5rem] text-center space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <h3 className="font-black text-xl text-gray-900 tracking-tight">Ready to Calculate</h3>
                                        <p className="text-sm text-gray-400 font-medium max-w-[240px] mx-auto leading-relaxed">
                                            Enter the base salary and attendance data on the left to see the calculated wage.
                                        </p>
                                    </div>
