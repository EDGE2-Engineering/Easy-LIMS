...
import { Plus, Edit, Trash2, Save, Search, AlertCircle, Ruler } from 'lucide-react';
  ...
  const handleSave = async () => {
    if (!editingUnitType.unit_type.trim()) {
      toast({
        ...
        description: 'Unit type name cannot be empty.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      if (isAddingNew) {
        ...
        toast({
          ...
          description: 'New unit type has been successfully added.',
        });
      } else {
    ...
    if (deleteConfirmation.id) {
      try {
        ...
        toast({
          ...
          description: 'The unit type has been removed.',
          variant: 'destructive',
        });
      } catch (error) {
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm animate-in slide-in-from-right-4 duration-300">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">
            {isAddingNew ? 'Add New Unit Type' : 'Edit Unit Type'}
          </h2>
        ...
        <div className="space-y-4 max-w-md">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setEditingUnitType({ ...editingUnitType, unit_type: e.target.value })
              }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1 uppercase text-[10px] tracking-widest ml-1">
            Manage measurement unit types for services
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Tooltip>
          ...
          <TooltipContent className="bg-gray-900 text-white border-gray-800">
            <p className="text-xs">Define a new measurement unit for services</p>
          </TooltipContent>
        </Tooltip>
      </div>
