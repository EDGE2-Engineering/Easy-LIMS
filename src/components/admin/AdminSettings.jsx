...
const PasswordField = ({
  ...
  <div className="space-y-2">
    <Label htmlFor={id} className={required ? 'text-primary font-semibold' : ''}>
      {label}
    </Label>
    <div className="relative">
      <Icon
        className={`absolute left-3 top-3 h-4 w-4 ${required ? 'text-primary' : 'text-gray-500'}`}
      />
      <Input
        id={id}
        name={id}
    ...
    reader.onload = (event) => {
      try {
        ...
        if (backup.pageImages)
          localStorage.setItem(STORAGE_KEYS.IMAGES, JSON.stringify(backup.pageImages));

        toast({
          title: 'Restore Successful',
          description: 'Site data has been restored. Reloading...',
        });
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">
                  To change your username, please contact the system administrator.
                </p>
              </div>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-4">
              <div className="p-4 bg-blue-50 text-blue-800 rounded-lg text-sm border border-blue-100">
                ...
                Since this app doesn't use a cloud database yet, all your changes (images, text,
                data) are stored in your browser.
                <strong> Download a backup regularly</strong> to ensure you don't lose data when
                clearing cache or switching devices.
              </div>
