...
import { 
    LayoutDashboard, FileText, IndianRupee, Clock, 
    CheckCircle2, AlertCircle, Zap, Wallet, TrendingUp,
    FileCheck, FileClock, Receipt, Calendar
} from 'lucide-react';
        ...
        return (
            <div className="h-[70vh] flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm font-bold text-gray-400 uppercase tracking-widest animate-pulse">Loading Accounts Dashboard...</p>
                </div>
            </div>
        );
    }
                                ...
                                <div className="divide-y divide-gray-50">
                                    {myDocuments.slice(0, 5).length === 0 ? (
                                        ...
                                        myDocuments.slice(0, 5).map((doc) => (
                                            <div key={doc.id} className="p-4 hover:bg-gray-50/50 transition-colors flex justify-between items-center group">
                                                <div className="flex items-center gap-3">
                                                    <div className={`p-2 rounded-lg ${doc.document_type === 'Quotation' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'}`}>
                                                        <FileText className="w-4 h-4" />
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-bold text-gray-900">{doc.quote_number}</p>
                                                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-tight">{doc.document_type} • {doc.clients?.client_name}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <p className="text-sm font-black text-gray-900"><Rupee />{(doc.content?.total || 0).toLocaleString()}</p>
                                                    <Button 
                                                        size="icon" 
                                                        variant="ghost" 
