...
const AdminOverburdenCorrectionManager = () => {
    ...
    const chartData = rows
        .filter(r => r.pressure !== '' && r.correction !== '' &&
            !isNaN(parseFloat(r.pressure)) && !isNaN(parseFloat(r.correction)))
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.error(error);
            toast({ title: 'Error', description: 'Failed to save correction data.', variant: 'destructive' });
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="space-y-8 max-w-4xl mx-auto pb-12">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b">
                        <tr>
                            <th className="text-center py-4 px-4 font-bold text-gray-400 uppercase tracking-widest text-[10px] w-12">#</th>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                            Hover anywhere for interpolated values &nbsp;·&nbsp;
                            Dense grid lines at 0.1 intervals between 0.5 – 1.0, coarse 0.5 steps elsewhere
                        </p>
                    </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-primary/10 rounded-xl border border-primary/20">
                ...
                <p className="text-xs text-gray-600 leading-relaxed">
                    Grid lines follow the reference diagram — dense 0.1 intervals between <strong>0.5 and 1.0</strong>,
                    coarse 0.5 steps outside. Y-axis is inverted (0 kN/m² at top, increasing downward).
                    Hover the curve for <strong>interpolated CF and Q values</strong>. Click <em>Save</em> to persist.
                </p>
            </div>
        </div>
    );
