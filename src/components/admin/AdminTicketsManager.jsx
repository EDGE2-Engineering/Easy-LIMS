...
const SQL_SCHEMA = `-- Create tickets table
...
export default function AdminTicketsManager({ id: propId }) {
  ...
  const handleCopySchema = () => {
    ...
    setCopied(true);
    toast({ title: 'SQL Schema Copied', description: 'Run it in Supabase SQL Editor.' });
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      ...
      setEditTicketForm((prev) => ({ ...prev, status: newStatus }));
    } catch (err) {
  ...
  const getAllowedStatuses = (ticket) => {
    if (!ticket) return [];
    if (isAdmin()) return ['Open', 'In Progress', 'Resolved', 'Invalid', 'Closed'];
    if (ticket.created_by === user.id) return ['Open', 'Resolved', 'Invalid'];
    return [];
  };
      ...
      <div className="max-w-4xl mx-auto py-12 px-6">
        <div className="bg-white rounded-3xl border border-gray-100 shadow-xl p-8 md:p-12 text-center">
          ...
          <p className="text-gray-500 font-medium max-w-lg mx-auto mb-8 text-sm leading-relaxed">
            The database tables <code>tickets</code> and <code>ticket_comments</code> do not exist
            yet. Copy the SQL below and run it in the <strong>SQL Editor</strong> of your Supabase
            Console.
          </p>
          <div className="relative text-left rounded-2xl overflow-hidden border border-gray-200 bg-slate-900 mb-8 max-h-[300px] overflow-y-auto">
            ...
            <pre className="p-6 text-xs text-slate-300 font-mono leading-relaxed whitespace-pre">
              {SQL_SCHEMA}
            </pre>
          </div>
          <Button
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-md overflow-hidden">
          ...
          <form onSubmit={handleCreateTicket} className="p-8 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                placeholder="Provide detailed description of the issue, steps to reproduce, or requirements..."
                value={newTicket.description}
                onChange={(e) => setNewTicket({ ...newTicket, description: e.target.value })}
                className="min-h-[160px] rounded-xl border-gray-100 bg-gray-50/50 resize-none text-sm font-semibold p-4"
              />
            </div>
  ...
  if (ticketIdParam) {
    ...
    return (
      <div className="w-full py-6 px-6 bg-white dark:bg-slate-900/40 border border-gray-150 dark:border-slate-800/80 backdrop-blur-md shadow-sm rounded-3xl animate-in fade-in duration-300 font-sans text-slate-800 dark:text-slate-200 font-sans">
        ...
        <div className="flex flex-wrap items-center gap-2 pb-4 border-b border-slate-100 dark:border-slate-800 mb-6">
          ...
          {/* Department Badge */}
          <span className="inline-flex items-center h-8 px-3 rounded-lg text-xs font-semibold border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 text-slate-600 dark:text-slate-300">
            Dept: {ticketDetails.department}
          </span>

          {/* Priority Badge */}
          <span
            className={`inline-flex items-center h-8 px-3 rounded-lg text-xs font-semibold capitalize border ${
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="space-y-2">
              ...
              {editable ? (
                  ...
                  {editingDesc ? (
                    <div className="space-y-2">
                      <Textarea
                        ...
                        className="min-h-[140px] w-full border border-blue-500 dark:border-blue-600 rounded-xl bg-white dark:bg-slate-950/60 text-slate-800 dark:text-slate-250 text-sm p-3.5 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/30 outline-none resize-y"
                    ...
                    <div
                      ...
                      className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap hover:bg-slate-50 dark:hover:bg-slate-800/40 p-3.5 rounded-xl border border-transparent hover:border-slate-250 dark:hover:border-slate-800 cursor-pointer min-h-[60px] transition-all"
                    >
                ...
                <div className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap p-3.5 bg-slate-50/50 dark:bg-slate-900/30 rounded-xl border border-slate-100 dark:border-slate-800/60 min-h-[60px]">
                  {ticketDetails.description || (
              ...
              {editable && (
                <div className="relative">
                  ...
                  <div
                    ...
                    className="h-20 border-dashed border-2 rounded-xl flex flex-col items-center justify-center border-slate-200 dark:border-slate-800 hover:border-blue-500 dark:hover:border-blue-600 hover:bg-blue-50/20 dark:hover:bg-blue-950/10 text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all cursor-pointer"
                  >
              ...
              {ticketDetails.attachments?.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 pt-1">
                  {ticketDetails.attachments.map((file, idx) => (
                    <div
                      key={idx}
                      className="relative overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/30 flex flex-col group h-40 transition-shadow hover:shadow-sm"
                    >
                      {/* Top Preview Area */}
                      {isImageFile(file) ? (
                        <div
                          className="flex-1 w-full overflow-hidden bg-slate-100 dark:bg-slate-950 flex items-center justify-center cursor-pointer"
                        ...
                        <div
                          className="flex-1 w-full overflow-hidden bg-slate-100 dark:bg-slate-950 flex flex-col items-center justify-center cursor-pointer gap-1.5"
                      ...
                      {/* Bottom Info Bar */}
                      <div className="bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800/80 p-2.5 flex items-center justify-between gap-1.5 shrink-0">
                ...
                {loadingComments ? (
                  ...
                  comments.map((comment) => (
                    <div key={comment.id} className="flex gap-3 items-start text-xs">
                      {/* Avatar */}
                      <div className="w-7 h-7 rounded-full bg-blue-100 dark:bg-blue-950/60 text-blue-800 dark:text-blue-400 font-bold uppercase flex items-center justify-center shrink-0 border border-blue-200 dark:border-blue-900/50">
                        {(comment.author?.full_name || 'U').substring(0, 2)}
                      </div>

                      <div className="flex-1 bg-slate-50 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800/80 p-3.5 rounded-2xl min-w-0">
                        ...
                        {comment.attachments?.length > 0 && (
                          <div className="flex flex-wrap gap-2 mt-2 pt-2 border-t border-slate-200/50 dark:border-slate-800/50">
                            {comment.attachments.map((file, idx) => (
                              <div
                                key={idx}
                                onClick={() => handleDownloadAttachment(file)}
                                className="flex items-center gap-1 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900 hover:text-blue-600 dark:hover:text-blue-400 transition-all cursor-pointer text-[10px] font-bold text-slate-600 dark:text-slate-400 truncate max-w-[150px]"
                                title={file.name}
                              >
                                <Paperclip className="w-3 h-3 text-slate-400 mr-0.5" />
              ...
              <form onSubmit={handleAddComment} className="flex gap-3 pt-2 items-start">
                ...
                <div className="flex-1 space-y-2">
                  <Textarea
                    ...
                    className="min-h-[85px] w-full border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-950/60 text-slate-800 dark:text-slate-200 text-xs p-3 resize-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/30 outline-none"
                  />
                      ...
                      {commentAttachments.map((f, i) => (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setCommentAttachments((prev) => prev.filter((_, idx) => idx !== i))
                            }
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <Button
                      type="submit"
                      disabled={
                        submittingComment || (!newComment.trim() && commentAttachments.length === 0)
                      }
                ...
                <span className="text-slate-800 dark:text-slate-200 font-bold col-span-2 capitalize">
                  {editable ? (
                    <div className="w-full">
                      ...
                      >
                        <SelectTrigger className="w-full h-9 text-xs bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 rounded-lg">
