  ...
  const handleSave = async () => {
    if (!editingTerm.text.trim()) {
      toast({
        ...
        description: 'Terms and Conditions text cannot be empty.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      if (isAddingNew) {
        ...
        toast({
          ...
          description: 'New Terms and Conditions has been successfully added.',
        });
      } else {
        await updateTerm(editingTerm.id, editingTerm.text, editingTerm.type);
        toast({
          title: 'Terms and Conditions Updated',
          description: 'Terms and Conditions has been updated.',
        });
      }
      setEditingTerm(null);
      setIsAddingNew(false);
  ...
  const confirmDelete = async () => {
    if (deleteConfirmation.id) {
      try {
        ...
        toast({
          title: 'Terms and Conditions Deleted',
          description: 'The Terms and Conditions has been removed.',
          variant: 'destructive',
        });
      } catch (error) {
        ...
        toast({
          title: 'Error',
          description: 'Failed to delete Terms and Conditions.',
          variant: 'destructive',
        });
      }
    }
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm animate-in slide-in-from-right-4 duration-300">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">
            {isAddingNew ? 'Add New Terms and Conditions' : 'Edit Terms and Conditions'}
          </h2>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1 uppercase text-[10px] tracking-widest ml-1">
            Manage standard clauses for documents
          </p>
        </div>
      </div>
