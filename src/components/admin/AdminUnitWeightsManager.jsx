...
const AdminUnitWeightsManager = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast({ title: 'Unit Weights Saved', description: 'Material unit weights have been updated successfully.' });
        } catch (error) {
            console.error(error);
            toast({ title: 'Error', description: 'Failed to save unit weights.', variant: 'destructive' });
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="space-y-8 max-w-6xl mx-auto pb-12">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b">
                        <tr>
                            <th className="text-left py-4 px-4 font-bold text-gray-400 uppercase tracking-widest text-[10px]">Material Type</th>
                            <th className="text-center py-4 px-4 font-bold text-gray-400 uppercase tracking-widest text-[10px]">Bulk Unit Weight (kN/m³)</th>
                            <th className="text-center py-4 px-4 font-bold text-gray-400 uppercase tracking-widest text-[10px]">Effective Unit Weight (kN/m³)</th>
                        </tr>
                    </thead>
                            ...
                            return (
                                <tr key={mat.id} className="hover:bg-gray-50/50 transition-colors group">
                                    ...
                                    <td className="py-4 px-4 text-center">
                                        <span className={`font-mono font-bold ${effectiveVal === '—' ? 'text-gray-300' : 'text-primary'}`}>
                                            {effectiveVal}
                                        </span>
                                    </td>
                                </tr>
            ...
            <div className="flex items-start gap-3 p-4 bg-primary/10 rounded-xl border border-primary/20">
                ...
                <p className="text-xs text-gray-600 leading-relaxed">
                    <strong>Note:</strong> Effective Unit Weight is automatically calculated as <strong>Bulk Unit Weight − 10 kN/m³</strong>. 
                    Enter the Bulk Unit Weight for each material and click <em>Save Unit Weights</em> to persist your changes.
                </p>
            </div>
        </div>
    );
